...
(defconst roc-description
  "Color theme by Ethan Schoonover, created 2011-03-24.
Ported to Emacs by Greg Pfeil, http://ethanschoonover.com/solarized.")

(defcustom roc-degrade nil
  "For test purposes only; when in GUI mode, forces ROC to use the 256
degraded color mode to test the approximate color values for accuracy."
  :type 'boolean
  :group 'roc)

(defcustom roc-diff-mode 'normal
  "Sets the level of highlighting to use in diff-like modes."
  :type 'symbol
  :options '(high normal low)
  :group 'roc)

(defcustom roc-bold t
  "Stops ROC from displaying bold when nil."
  :type 'boolean
  :group 'roc)

(defcustom roc-underline t
  "Stops ROC from displaying underlines when nil."
  :type 'boolean
  :group 'roc)

(defcustom roc-italic t
  "Stops ROC from displaying italics when nil."
  :type 'boolean
  :group 'roc)
...
(defcustom roc-broken-srgb (if (and (eq system-type 'darwin)
                                          (eq window-system 'ns))
                                     t
                                   nil)
  "Emacs bug #8402 results in incorrect color handling on Macs. If this is t
\(the default on Macs), ROC works around it with alternative colors.
However, these colors are not totally portable, so you may be able to edit
the \"Gen RGB\" column in roc-definitions.el to improve them further."
  :type 'boolean
  :group 'roc)
...
(defun roc-color-definitions (mode)
  (flet ((find-color (name)
    ...
    (let ((base03      (find-color 'base03))
          ...
          (bold        (if roc-bold 'bold 'normal))
          (bright-bold (if roc-bold 'normal 'bold))
          (underline   (if roc-underline t nil))
          (opt-under   nil)
          (italic      (if roc-italic 'italic 'normal)))
      (when (eq 'light mode)
...
(defmacro create-roc-theme (mode)
  (let* ((theme-name (intern (concat "roc-" (symbol-name mode))))
         (defs (roc-color-definitions mode))
         (theme-vars (mapcar (lambda (def) (list (car def) (cdr def)))
                             (second defs)))
         (theme-faces (first defs)))
    `(progn
       (deftheme ,theme-name ,roc-description)
